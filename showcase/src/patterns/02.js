...
const useClapAnimation = ({
  ...
  useEffect(() => {
    ...
    if (typeof clapEl === 'string') {
      const clap = document.querySelector(clapEl)
...
const ClapCount = ({ count, setRef }) => {
  return (
    <span data-refkey='clapCountRef' className={styles.count}>
      + {count}
    </span>
  )
}
...
const CountTotal = ({ countTotal, setRef }) => {
  return (
    <span data-refkey='clapTotalRef' className={styles.total}>
      {countTotal}
    </span>
  )
}
